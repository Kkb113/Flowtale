...
This verifies the API with pinned Maven/Java 17, checks generated contracts, builds the Node 22 frontend and FFmpeg worker, applies MySQL/PostgreSQL migrations, initializes local object storage/queues, seeds development settings and waits for service readiness. The first run downloads images and dependencies. Open [local Fable](http://localhost:3000/login) and select **Continue to Fable**. This uses a dedicated manual-testing identity, separate from automated test workspaces. No passwords or Auth0 keys are needed. The existing automated accounts remain available at `/login?testing=1`; do not use the manual identity for automated mutations. Fonts and default images are bundled with the app, and development banners are omitted from product pages. Local billing remains disabled and entitlements remain simulated.

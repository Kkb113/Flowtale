...
| Capture transfer | Implement durable acknowledgement/session integrity and duplicate/restart handling without adding recovery steps to ordinary recording. Preserve the original bounded five-second quiet wait for unavailable embedded frames: a screen still requires its root document and screenshot, and transfer acknowledgement still requires committed client storage. Time alone must never manufacture missing primary capture data or acknowledge an unsaved transfer. Keep independent recordings isolated through handoff, creation, retry and cleanup. |
